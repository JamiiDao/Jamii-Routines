        ...
        <>
            <div className="flex flex-col flex-1 items-center justify-center w-full px-4">
                <div className="flex flex-col text-white w-full justify-center items-center max-w-lg rounded-2xl bg-secondary p-10 shadow-xl">
                    ...
                    {(passkeyStatus.state === PasskeyOpStatus.Pending) && (<div className="text-lg text-center mt-10 text-normal">
                    ...
                    {(passkeyStatus.state === PasskeyOpStatus.Connect && passkeyStatus.error !== "") &&
                        (<div className="bg-red-600 px-4 py-2 w-full mt-10 rounded-lg font-bold text-lg text-wrap">{passkeyStatus.error}</div>)
                    }

                    {(passkeyStatus.state === PasskeyOpStatus.Connect && passkeyStatus.error !== "") &&
                        (
                            <ActionButton onClick={handleTryAgainConnectPasskey} bgBackground="bg-complement">
                                Try Again
                            </ActionButton>
                        )
                    }
                ...
                {(passkeyStatus.state === PasskeyOpStatus.Pending) && (
